        ...
        PseudoInfiniteViewport.Pannable
{
    ...
    private static final int DEFAULT_CELL_SIZE = 47;
     ...
     * Highlights the End panel will a yellow rectangle.
     * @param g2 the graphics context
     */
    private void drawOccupants(Graphics2D g2)
    {
        ArrayList<Location> occupantLocs = grid.getOccupiedLocations();
        for (Location loc : occupantLocs) {
            int end = Human.getEndLocation().getCol();
            int end2 = Human.getEndLocation().getRow();
            Rectangle rectangle = new Rectangle();
            rectangle.setBounds(end * 48, end2 * 48, 48, 48);
    ...
    {
        String versionId = "";
        if ("1.00".compareTo(versionId) == 0) return;
     ...
     * mouse. This method is used by the GUI when creating Actor by clicking on
    ...
    private String getToolTipText(Location loc)
    {
        ...
        if (f != null)
            ...
            return MessageFormat.format(resources
                    .getString("cell.tooltip.empty"), loc, null);
    }
